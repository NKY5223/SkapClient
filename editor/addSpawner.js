function addSpawner() {
    ...
    function mousedown(e) {
        ...
        let spawner = createSpawner(posX, posY, 0, 0);
        currentArea.objects.spawner.push(spawner);
        objectmenu.appendChild(spawner.element);
        if (selectedObject) hide(selectedObject.element);
        selectedObject = spawner;

        function mousemove(e) {
            let x = Math.round((e.offsetX - canvas.width / 2) / camScale + camX);
            let y = Math.round((e.offsetY - canvas.height / 2) / camScale + camY);
            spawner.size.x = Math.max(x - posX, 0);
            spawner.size.y = Math.max(y - posY, 0);
            spawner.inputs.w.value = spawner.size.x;
            spawner.inputs.h.value = spawner.size.y;
        }
