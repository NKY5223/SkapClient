function addGravZone() {
    ...
    function mousedown(e) {
        ...
        let gravZone = createGravZone(posX, posY, 0, 0);
        currentArea.objects.gravityZone.push(gravZone);
        objectmenu.appendChild(gravZone.element);
        if (selectedObject) hide(selectedObject.element);
        selectedObject = gravZone;

        function mousemove(e) {
            let x = Math.round((e.offsetX - canvas.width / 2) / camScale + camX);
            let y = Math.round((e.offsetY - canvas.height / 2) / camScale + camY);
            gravZone.size.x = Math.max(x - posX, 0);
            gravZone.size.y = Math.max(y - posY, 0);
            gravZone.inputs.w.value = gravZone.size.x;
            gravZone.inputs.h.value = gravZone.size.y;
        }
